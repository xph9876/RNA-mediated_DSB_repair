...
def main():
  ...
  for l in args.sam:
    ...
    if indel == 0:
      ...
      sequence = get_near_ref(ws, ref, args.min_length, mutation, args.dsb)
      if sequence != None:
        sequences[sequence, cigar] += 1
      else:
        seq = ws[9]
  ...
  fw.write('Sequence\tCIGAR\tCount\n')
  for s, cigar in seqs:
    fw.write(f'{s.replace("-", "")}\t{cigar}\t{sequences[s, cigar]}\n')
