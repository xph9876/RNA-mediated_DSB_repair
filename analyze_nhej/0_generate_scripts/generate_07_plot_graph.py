...
if __name__ == '__main__':
  for ext in ['sh', 'ps1']:
    for img_ext in ['png', 'html']:
      with open(os.path.join('run_07_plot_graph_' + img_ext + os.path.extsep + ext), 'w') as file_out:
        for info in generate_constants.EXPERIMENT_INFO.to_dict('records'):
          for subst_type in library_constants.SUBST_TYPES:
            ...
            arg_range_x = get_arg_range_x(generate_constants.LAYOUT_NAME, info['layout_group'])
            arg_range_y = get_arg_range_y(generate_constants.LAYOUT_NAME, info['layout_group'])
            if generate_constants.USE_COMMON_LAYOUT:
              arg_precomputed_layout_dir = (
                ' --precomputed_layout_dir ' +
                generate_06_precomputed_layout.get_output_dir(generate_constants.LAYOUT_NAME, info['layout_group'])
              )
            else:
              arg_precomputed_layout_dir = ''
