...
def filter_adapter(sequence, tagF, tagR, q_score):
	...
	non_library_R = 0
	non_library_F = 0
	if sequence[0:12].find('GGAAGT') != int(-1):
		s1 = sequence[0:12].find('GGAAGT')
		tagRr = sequence[s1-4:s1] # tagFr is actual sequence of tag in each read.
		if tagRr == tagR:
			if sequence[-12:].find('CTTGAA') != int(-1):
				s2 = sequence[-12:].find('CTTGAA')
				if s2 >= 2:
					tagFr = sequence[-6+s2:]
				else:
					tagFr = sequence[-6+s2:-2+s2]
...
def main():
	...
	parser.add_argument('-tagF', type= str, help='tag complementary sequence of forward primer')
	parser.add_argument('-tagR', type= str, help='tag sequence of reverse primer')
